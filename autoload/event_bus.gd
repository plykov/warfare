...
signal keybind_changed(action: StringName, physical_keycode: int)
...
signal campaign_legacy_garden_loaded(mission_index: int, state: Dictionary)
signal campaign_pride_loaded(value: float)
signal restoration_feedback_changed(purity: float, bloom_count: int, persisted: bool)
signal restoration_legacy_changed(source_count: int, bloom_count: int, mean_purity: float)
